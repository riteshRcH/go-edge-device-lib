...
func (cg *BasicConnectionGater) loadRules(ctx context.Context) error {
	...
	if err != nil {
		log.Error(fmt.Sprintf("error querying datastore for blocked peers: %s", err))
		return err
	}

	for r := range res.Next() {
		if r.Error != nil {
			log.Error(fmt.Sprintf("query result error: %s", r.Error))
			return err
		}
	...
	if err != nil {
		log.Error(fmt.Sprintf("error querying datastore for blocked addrs: %s", err))
		return err
	}
	...
	if err != nil {
		log.Error(fmt.Sprintf("error querying datastore for blocked subnets: %s", err))
		return err
	}
	...
	for r := range res.Next() {
		...
		if err != nil {
			log.Error(fmt.Sprintf("error parsing CIDR subnet: %s", err))
			return err
		}
...
func (cg *BasicConnectionGater) BlockPeer(p peer.ID) error {
	if cg.ds != nil {
		...
		if err != nil {
			log.Error(fmt.Sprintf("error writing blocked peer to datastore: %s", err))
			return err
		}
	}
...
func (cg *BasicConnectionGater) UnblockPeer(p peer.ID) error {
	if cg.ds != nil {
		...
		if err != nil {
			log.Error(fmt.Sprintf("error deleting blocked peer from datastore: %s", err))
			return err
		}
	}
...
func (cg *BasicConnectionGater) BlockAddr(ip net.IP) error {
	if cg.ds != nil {
		...
		if err != nil {
			log.Error(fmt.Sprintf("error writing blocked addr to datastore: %s", err))
			return err
		}
	}
...
func (cg *BasicConnectionGater) UnblockAddr(ip net.IP) error {
	if cg.ds != nil {
		...
		if err != nil {
			log.Error(fmt.Sprintf("error deleting blocked addr from datastore: %s", err))
			return err
		}
	}
...
func (cg *BasicConnectionGater) UnblockSubnet(ipnet *net.IPNet) error {
	if cg.ds != nil {
		...
		if err != nil {
			log.Error(fmt.Sprintf("error deleting blocked subnet from datastore: %s", err))
			return err
		}
	}
...
func (cg *BasicConnectionGater) InterceptAddrDial(p peer.ID, a ma.Multiaddr) (allow bool) {
	...
	ip, err := manet.ToIP(a)
	if err != nil {
		log.Warn(fmt.Sprintf("error converting multiaddr to IP addr: %s", err))
		return true
	}
