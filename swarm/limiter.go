...
func (dl *dialLimiter) freeFDToken() {
	log.Debug(fmt.Sprintf("[limiter] freeing FD token; waiting: %d; consuming: %d", len(dl.waitingOnFd), dl.fdConsuming))
	dl.fdConsuming--
...
func (dl *dialLimiter) freePeerToken(dj *dialJob) {
	log.Debug(fmt.Sprintf("[limiter] freeing peer token; peer %s; addr: %s; active for peer: %d; waiting on peer limit: %d",
...
func (dl *dialLimiter) addCheckFdLimit(dj *dialJob) {
	if dl.shouldConsumeFd(dj.addr) {
		if dl.fdConsuming >= dl.fdLimit {
			log.Debug(fmt.Sprintf("[limiter] blocked dial waiting on FD token; peer: %s; addr: %s; consuming: %d; "+
				"limit: %d; waiting: %d", dj.peer, dj.addr, dl.fdConsuming, dl.fdLimit, len(dl.waitingOnFd)))
			dl.waitingOnFd = append(dl.waitingOnFd, dj)
			return
		}

		log.Debug(fmt.Sprintf("[limiter] taking FD token: peer: %s; addr: %s; prev consuming: %d",
			dj.peer, dj.addr, dl.fdConsuming))
		// take token
		dl.fdConsuming++
	}

	log.Debug(fmt.Sprintf("[limiter] executing dial; peer: %s; addr: %s; FD consuming: %d; waiting: %d",
...
func (dl *dialLimiter) addCheckPeerLimit(dj *dialJob) {
	if dl.activePerPeer[dj.peer] >= dl.perPeerLimit {
		log.Debug(fmt.Sprintf("[limiter] blocked dial waiting on peer limit; peer: %s; addr: %s; active: %d; "+
...
func (dl *dialLimiter) AddDialJob(dj *dialJob) {
	...
	log.Debug(fmt.Sprintf("[limiter] adding a dial job through limiter: %v", dj.addr))
...
func (dl *dialLimiter) clearAllPeerDials(p peer.ID) {
	...
	log.Debug(fmt.Sprintf("[limiter] clearing all peer dials: %v", p))
