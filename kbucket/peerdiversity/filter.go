...
import (
	...
	"github.com/riteshRcH/go-edge-device-lib/cidranger"

	ma "github.com/riteshRcH/go-edge-device-lib/multiaddr"
	manet "github.com/riteshRcH/go-edge-device-lib/multiaddr/net"
)

var dfLog, _ = zap.NewProduction()
...
func (f *Filter) TryAdd(p peer.ID) bool {
	...
	if len(addrs) == 0 {
		dfLog.Debug(fmt.Sprintln("no addresses found for peer", "appKey", f.logKey, "peer", p.Pretty()))
		return false
	}
	...
	for _, a := range addrs {
		ip, err := manet.ToIP(a)
		if err != nil {
			dfLog.Error(fmt.Sprintln("failed to parse IP from multiaddr", "appKey", f.logKey,
		...
		if err != nil {
			dfLog.Error(fmt.Sprintln("failed to find Group Key", "appKey", f.logKey, "ip", ip.String(), "peer", p,
				"err", err))
			return false
		}
		if len(key) == 0 {
			dfLog.Error(fmt.Sprintln("group key is empty", "appKey", f.logKey, "ip", ip.String(), "peer", p))
			return false
		}
...
func (f *Filter) ipGroupKey(ip net.IP) (PeerIPGroupKey, error) {
	...
	case nil:
		...
		if len(s) == 0 {
			dfLog.Debug(fmt.Sprintln("ASN not known", "appKey", f.logKey, "ip", ip))
