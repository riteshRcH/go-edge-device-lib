...
func go_write_bio_write(b *C.BIO, data *C.char, size C.int) (rc C.int) {
	defer func() {
		if err := recover(); err != nil {
			logger.Fatal(fmt.Sprintf("openssl: writeBioWrite panic'd: %v", err))
			rc = -1
		}
	}()
	ptr := loadWritePtr(b)
...
func go_write_bio_ctrl(b *C.BIO, cmd C.int, arg1 C.long, arg2 unsafe.Pointer) (
	...
	defer func() {
		if err := recover(); err != nil {
			logger.Fatal(fmt.Sprintf("openssl: writeBioCtrl panic'd: %v", err))
			rc = -1
		}
	}()
	switch cmd {
...
func go_read_bio_read(b *C.BIO, data *C.char, size C.int) (rc C.int) {
	defer func() {
		if err := recover(); err != nil {
			logger.Fatal(fmt.Sprintf("openssl: go_read_bio_read panic'd: %v", err))
			rc = -1
		}
	}()
	ptr := loadReadPtr(b)
...
func go_read_bio_ctrl(b *C.BIO, cmd C.int, arg1 C.long, arg2 unsafe.Pointer) (
	...
	defer func() {
		if err := recover(); err != nil {
			logger.Fatal(fmt.Sprintf("openssl: readBioCtrl panic'd: %v", err))
			rc = -1
		}
	}()
	switch cmd {
